...
export class DriveFolderService {
  ...
  static async ensureCompanyFolder(companyName: string): Promise<{ folderId: string; folderUrl: string }> {
    const normalized = this.normalizeCompanyName(companyName);
    
    let activePromise = this.companyFolderPromises.get(normalized);
    if (!activePromise) {
      activePromise = this._ensureCompanyFolderInternal(companyName, normalized);
  ...
  private static async _ensureCompanyFolderInternal(companyName: string, normalized: string): Promise<{ folderId: string; folderUrl: string }> {
    ...
    if (rootFolderId) {
      ...
      const matchingFolders = await driveRest.listFiles(q);
    ...
    // Create new folder
    const folder = await driveRest.createFolder(companyName, rootFolderId ? [rootFolderId] : []);
  ...
  static async ensureSubFolder(parentFolderId: string, subFolderName: string): Promise<{ folderId: string; folderUrl: string }> {
    const key = `${parentFolderId}:${subFolderName.toLowerCase().replace(/\s+/g, ' ').trim()}`;
    
    let activePromise = this.subFolderPromises.get(key);
    if (!activePromise) {
      activePromise = this._ensureSubFolderInternal(parentFolderId, subFolderName);
  ...
  private static async _ensureSubFolderInternal(parentFolderId: string, subFolderName: string): Promise<{ folderId: string; folderUrl: string }> {
    const q = `'${parentFolderId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false`;
    const allSubFolders = await driveRest.listFiles(q);
    ...
    const folder = await driveRest.createFolder(subFolderName, [parentFolderId]);
  ...
  static async handleNewProject(projectData: any) {
    ...
    try {
      ...
      const profileData = await firestoreRest.getDocument('profiles', clientId);
      if (!profileData) {
      ...
      const companyFolder = await this.ensureCompanyFolder(companyName);

      // 3. Ensure Subfolder
      const subFolderName = this.buildSubFolderName(jobType, new Date());
      const subFolder = await this.ensureSubFolder(companyFolder.folderId, subFolderName);
