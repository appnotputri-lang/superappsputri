...
const PendirianNewList: React.FC<{
  ...
  useEffect(() => {
    // USING NEW COLLECTION: pendirian_new_projects
    const q = query(collection(db, 'pendirian_new_projects'));
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      console.error("Firestore Error in PendirianNewList:", error);
    });
    return () => unsub();
  }, []);
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Yakin ingin menghapus data ini?')) {
      ...
      await deleteDoc(doc(db, 'pendirian_new_projects', id));
      
      try {
        const notifId = crypto.randomUUID();
        await setDoc(doc(db, 'notifications', notifId), {
          id: notifId,
          title: 'Pendirian PT (NEW) Dihapus',
          description: `Data Pendirian PT (NEW) untuk perusahaan "${targetCompanyName}" telah berhasil dihapus.`,
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white border border-slate-200 rounded-sm p-6 shadow-sm gap-4">
        <div className="flex items-center gap-4">
          <div className="bg-[#0c2444] p-3 rounded-2xl shadow-md flex items-center justify-center relative overflow-hidden group">
            {/* Some distinct visual for NEW */}
            <div className="absolute inset-0 bg-blue-500/20 group-hover:scale-110 transition-transform"></div>
            <FileText className="w-8 h-8 text-white relative z-10" />
            <span className="absolute -bottom-1 -right-1 text-[8px] font-black bg-yellow-400 text-slate-800 px-1 pt-2 pb-0.5 rounded-tl object-contain z-20">NEW</span>
          </div>
          <div>
            <h1 className="text-xl font-extrabold text-slate-900 tracking-tight font-sans">Pendirian PT (NEW)</h1>
            <p className="text-slate-500 text-xs">Generator Akta Pendirian dengan Engine DOCX Template.</p>
          </div>
        </div>
        <button
          onClick={onAdd}
      ...
      <div className="bg-white border border-slate-200 rounded-sm p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pb-2 border-b border-slate-100">
          <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Tabel Data Pendirian PT (NEW)</h2>
...
export default PendirianNewList;
