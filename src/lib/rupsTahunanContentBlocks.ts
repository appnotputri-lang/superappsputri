  ...
  if (hasAmendments) {
    data.amendmentDeeds!.forEach((deed, i) => {
      ...
      blocks.push({
        ...
        runs: [
          {
            text: `Akta tertanggal ${tglDeedFormatted} Nomor ${deed.number} yang dibuat di hadapan ${checkNotaryWording(deed.notary, deed.notaryTitle, deed.notaryDomicile)}${skText ? " yang " + skText : ""}${isLast ? "." : ";"}`,
          },
        ],
      });
    });
