...
export const generateRupstAktaBlocks = (data: CompanyData): Block[] => {
  ...
  if (data.representativeType === "EXISTING") {
    const found = data.shareholders.find(
    ...
    if (found) {
      ...
      const chairSh = data.shareholders.find(
        (s) =>
          (s.name || "").toUpperCase() ===
          (data.meetingChair || "").toUpperCase(),
      );
      rep =
        chairSh?.isProxy && chairSh.proxyData
          ? chairSh.proxyData
          : chairSh || data.shareholders[0];
    }
  } else {
  ...
  const getPersonDetailRuns = (person: any): FormatToken[] => {
    ...
    const currentSal = !isBadanHukum
      ? (person?.salutation || "Tuan").trim()
      : "";
    ...
    let detailText = person
      ? formatPersonDetails(person, tglAngka, tglHuruf, true)
      : `, lahir di ..., pada tanggal ... (...), Warga Negara Indonesia, swasta, bertempat tinggal di ..., Rukun Tetangga ..., Rukun Warga ..., Kelurahan ..., Kecamatan ..., pemegang Kartu Tanda Penduduk Nomor ...`;
  ...
  const establishmentDeedDateText = formatAktaDate(
    data.establishmentDeedDate || "",
  );
  ...
  const isSingleAmendment =
    data.amendmentDeeds && data.amendmentDeeds.length === 1;
  ...
  const getFoundationRuns = (suffix: string = ""): FormatToken[] => {
    ...
    return [
      ...
      { text: `, tanggal ` },
      {
        text: formatAktaDate(data.signingDate || ""),
  ...
  if (data.rupstType === "sirkuler") {
    blocks.push({
      ...
      runs: [
        {
          text: `Bahwa para pemegang saham perseroan telah mengambil keputusan berdasarkan Keputusan Para Pemegang Saham `,
        },
        { text: formatCompanyName(data.companyName), bold: true },
        {
          text: ` yang dibuat di bawah tangan, ditandatangani secara bersama-sama dan/atau dengan cara diedarkan, yang terakhir ditandatangani pada tanggal ${formatAktaDate(data.signingDate || "")} bermeterai cukup yang aslinya dilekatkan pada minuta akta ini, yang ditandatangani oleh:`,
        },
      ],
    });
  } else {
  ...
  if (data.rupstType === "sirkuler") {
    finalAttendees.forEach((att) => {
      att.management = null;
    });
    // Remove attendees that have no roles left
    finalAttendees = finalAttendees.filter(
      (att) =>
        att.ownShares ||
        (att.representations && att.representations.length > 0),
    );
  }

  finalAttendees.forEach((att, idx) => {
    ...
    if (totalSubBullets === 0) {
      ...
      if (att.management) {
        ...
        blocks.push({
          ...
          runs: [
            {
              text: `${toTitleCase(att.management.position)} Perseroan${isLast ? "." : ";"}`,
            },
          ],
        });
      }
  ...
  if (data.rupstType === "sirkuler") {
    blocks.push({
      ...
      runs: [
        ...
        {
          text: `${formatNumber(totalShares)} (${terbilang(totalShares)})`,
          bold: true,
        },
        {
          text: ` lembar saham atau 100 % (seratus persen) dari saham dalam Perseroan `,
        },
        { text: formatCompanyName(data.companyName), bold: true },
        {
    ...
    blocks.push({
      ...
      runs: [
        ...
        {
          text: `Rp. ${formatNumber(totalNominal)},- (${terbilang(totalNominal)} rupiah)`,
          bold: true,
        },
        { text: ` telah hadir dalam rapat ini sebanyak ` },
        {
          text: `${formatNumber(presentShares)} (${terbilang(presentShares)})`,
          bold: true,
        },
        { text: ` lembar saham perseroan atau senilai ` },
        {
          text: `Rp. ${formatNumber(presentNominal)},- (${terbilang(presentNominal)} rupiah)`,
          bold: true,
        },
  ...
  if (data.rupstType !== "sirkuler") {
    blocks.push({
      ...
      runs: [
        { text: `Bahwa menurut ` },
        {
          text: `Pasal ${data.rupstQuorumArticle || "9"} ayat ${data.rupstQuorumParagraph || "4"} Anggaran Dasar Perseroan`,
          color: "FF0000",
        },
        {
  ...
  let chairNameValue = (data.meetingChair || rep?.name || "RAJANDRAN SHUNMUGAM")
    .trim()
    .toUpperCase();
  ...
  if (chairNameValue) {
    const foundSh = data.shareholders.find(
      (s) =>
        ...
        stripSalutation(chairNameValue),
    );
    if (foundSh) {
      ...
      const foundNewMgmt = data.newManagementItems?.find(
        (m) =>
          stripSalutation((m.name || "").toUpperCase()) ===
          stripSalutation(chairNameValue),
      );
      if (foundNewMgmt) {
        chairSalutation = foundNewMgmt.salutation || "Tuan";
      } else {
        const foundOldMgmt = data.oldManagementItems?.find(
          (m) =>
            stripSalutation((m.name || "").toUpperCase()) ===
            stripSalutation(chairNameValue),
        );
  ...
  if (data.rupstType !== "sirkuler") {
    ...
    blocks.push(
      ...
      },
      {
        ...
        runs: [
          {
            text: `Persetujuan Laporan Tahunan Perseroan Tahun Buku ${data.rupstFiscalYear || "2025"};`,
          },
        ],
      },
      {
        ...
        runs: [
          {
            text: `Pengesahan Laporan Keuangan Perseroan Tahun Buku ${data.rupstFiscalYear || "2025"};`,
          },
        ],
      },
      {
      ...
      },
      {
        ...
        runs: [
          {
            text: "Pemberian pelunasan dan pembebasan tanggung jawab sepenuhnya (acquit et de charge) kepada Direksi dan Komisaris.",
          },
        ],
      },
    );
  ...
  if (data.rupstAlasanAuditA !== false)
    reasonsAudit.push(
      `Kegiatan Usaha Perseroan ${data.rupstIsAudited ? "" : "tidak "}menghimpun dan/atau mengelola dana masyarakat.`,
    );
  if (data.rupstAlasanAuditB !== false)
    reasonsAudit.push(
      `Perseroan ${data.rupstIsAudited ? "" : "tidak "}menerbitkan surat pengakuan utang kepada masyarakat.`,
    );
  if (data.rupstAlasanAuditC !== false)
    reasonsAudit.push(
      `Perseroan ${data.rupstIsAudited ? "merupakan" : "tidak merupakan"} Perseroan Terbuka (Tbk).`,
    );
  if (data.rupstAlasanAuditD !== false)
    reasonsAudit.push(
      `Perseroan ${data.rupstIsAudited ? "merupakan" : "tidak merupakan"} Persero.`,
    );
  if (data.rupstAlasanAuditE !== false)
    reasonsAudit.push(
      `Aset dan/atau jumlah peredaran usaha ${data.rupstIsAudited ? "lebih" : "tidak lebih"} dari 50 Milyar, atau`,
    );
  if (data.rupstAlasanAuditF !== false)
    reasonsAudit.push(
      `${data.rupstIsAudited ? "" : "Tidak "}diwajibkan oleh peraturan perundang-undangan.`,
    );
  ...
  if (reasonsAudit.length === 1) {
    const reasonText = reasonsAudit[0].replace(", atau", "").replace(/\.$/, "");
    ...
    reasonsAudit.forEach((reason, idx) => {
      const isLast = idx === reasonsAudit.length - 1;
      const isSecondLast = idx === reasonsAudit.length - 2;
      
      const cleanReason = reason.replace(", atau", "").replace(/\.$/, "");
  ...
  if (data.rupstIsAudited) {
    ...
    const consistOfTarget =
      data.rupstType === "sirkuler"
        ? "Keputusan Sirkuler ini"
        : "Notulen Rapat Umum Pemegang Saham Tahunan ini";
    const consistOf = [
      `Laporan Keuangan, terlampir dan dilekatkan pada ${consistOfTarget}.`,
      `Laporan mengenai Kegiatan Perseroan, terlampir dan dilekatkan pada ${consistOfTarget}.`,
      `Laporan Pelaksanaan Tanggung Jawab Sosial dan Lingkungan, terlampir dan dilekatkan pada ${consistOfTarget}.`,
      `Rincian Masalah yang timbul selama tahun buku yang mempengaruhi kegiatan usaha perseroan, terlampir dan dilekatkan pada ${consistOfTarget}.`,
      `Laporan mengenai tugas pengawasan yang telah dilaksanakan oleh Dewan Komisaris selama tahun buku yang baru lampau, terlampir dan dilekatkan pada ${consistOfTarget}.`,
      `Nama Anggota Direksi dan Anggota Dewan Komisaris, terlampir dan dilekatkan pada ${consistOfTarget}.`,
      `Gaji dan Tunjangan bagi Anggota Direksi dan Gaji atau Honorarium dan Tunjangan bagi Anggota Dewan Komisaris Perseroan untuk Tahun yang baru lampau, terlampir dan dilekatkan pada ${consistOfTarget}.`,
    ];
    ...
    blocks.push({
      ...
      runs: [
        {
          text: `Menyetujui dan menerima dengan baik Laporan Tahunan Perseroan untuk tahun buku yang berakhir pada tanggal 31 Desember ${data.rupstFiscalYear || "2025"};`,
        },
      ],
    });
    decisionIndex++;
    ...
    const signatorySh = data.shareholders.find(
      (s) => s.name === data.rupstFinancialReportSignatoryName,
    );
    ...
    if (data.rupstStatementNeraca === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Laporan Keuangan, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
    if (data.rupstStatementLabaRugi === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Laporan mengenai Kegiatan Perseroan, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
    if (data.rupstStatementPerubahanEkuitas === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Laporan Pelaksanaan Tanggung Jawab Sosial dan Lingkungan, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
    if (data.rupstStatementArusKas === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Rincian Masalah yang timbul selama tahun buku yang mempengaruhi kegiatan usaha perseroan, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
    if (data.rupstStatementCatatan === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Laporan mengenai tugas pengawasan yang telah dilaksanakan oleh Dewan Komisaris selama tahun buku yang baru lampau, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
    if (data.rupstStatementNamaAnggota === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Nama Anggota Direksi dan Anggota Dewan Komisaris, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
    if (data.rupstStatementGaji === true) {
      blocks.push({
        ...
        runs: [
          {
            text: `Gaji dan Tunjangan bagi Anggota Direksi dan Gaji atau Honorarium dan Tunjangan bagi Anggota Dewan Komisaris Perseroan untuk Tahun yang baru lampau, terlampir dan dilekatkan pada ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler ini" : "Notulen Rapat Umum Pemegang Saham Tahunan ini"}.`,
          },
        ],
      });
    }
  ...
  if (data.rupstIsAudited) {
    ...
    if (!isNeg && data.rupstDividendAmount && data.rupstDividendAmount > 0) {
      ...
      const aktaDivBlocks: Block[] = divBlocks.map((b) => {
        if (b.type === "list") {
          ...
          if (
            b.runs.length > 0 &&
            b.runs[0].text.includes(
              "Memberikan persetujuan dan pengesahan atas tindakan Direksi",
            )
          ) {
            return {
              type: "p",
      ...
      if (data.rupstNetProfit !== undefined && data.rupstNetProfit !== null) {
        ...
        netProfitText = `Menetapkan penggunaan laba bersih Perseroan sebagaimana diusulkan dalam ${data.rupstType === "sirkuler" ? "Keputusan Sirkuler" : "Rapat"}.`;
      }
    ...
    const netProfitColor =
      data.rupstNetProfit !== undefined && data.rupstNetProfit !== null
        ? undefined
        : "FF0000";
    const dividendColor =
      data.rupstDividendAmount !== undefined &&
      data.rupstDividendAmount !== null
        ? undefined
        : "FF0000";
    ...
    if (
      ...
      const previousRetainedStr = `Rp${formatNumber(Math.abs(previousRetained))},- (${terbilang(Math.abs(previousRetained))} rupiah)`;
      ...
      const introRuns = showRetainedInfo
        ? [
            {
            ...
            {
              text: `, dengan saldo ${retainedLabel} ditahan Perseroan sampai dengan tahun buku ${prevYear} sebesar `,
            },
        ...
        : [
            {
              text: `Menetapkan penggunaan laba bersih Perseroan Tahun Buku ${fiscalYear} sebesar `,
            },
            { text: amtStr, color: netProfitColor },
            { text: `, dengan penggunaan sebagai berikut:` },
          ];
      ...
      if (divAmt > 0) {
        const afterDivRuns = showRetainedInfo
          ? [
              { text: "Laba bersih tahun berjalan sebesar " },
              { text: amtStr, color: netProfitColor },
              {
                text: ` ditambah saldo ${retainedLabel} ditahan tahun sebelumnya sebesar `,
              },
              { text: previousRetainedStr },
              {
                text: ` setelah dikurangi dividen, maka total saldo ${totalRetainedLabel} ditahan Perseroan menjadi sebesar `,
              },
              { text: totalLabaDitahanStr },
              {
                text: ` ditetapkan sebagai saldo ${totalRetainedLabel} ditahan Perseroan.`,
              },
            ]
          : [
              { text: "Laba bersih tahun berjalan sebesar " },
              { text: amtStr, color: netProfitColor },
              { text: ` setelah dikurangi dividen, sisanya sebesar ` },
              {
                text: `Rp. ${formatNumber(Math.abs(netProfit - divAmt))},- (${terbilang(Math.abs(netProfit - divAmt))} rupiah)`,
              },
            ...
            : [
                {
                  ...
                  runs: showRetainedInfo
                    ...
                    : [
                        {
                          text: "Seluruh laba bersih Perseroan dibukukan sebagai laba ditahan Perseroan.",
                        },
                      ],
                },
              ]),
  ...
  const overrideS1Alamat = (
    data.saksi1Alamat ||
    "Jalan Sukaresmi Nomor 17, RT. 005 RW. 005, Kecamatan Lembang, Desa Mekarwangi"
  ).replace(/Sukaresmi Nomor 12/gi, "Sukaresmi Nomor 17");
  const s1DetailRaw =
