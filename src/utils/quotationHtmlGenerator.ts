...
export function generateQuotationHTML(quotation: Quotation, qrBase64: string, autoPrint = false): string {
...
<body>
  <div class="page-container">
    ...
    <div class="section-grid">
      ...
      <div class="section-col">
        <div class="section-header">Penawaran Kepada</div>
    ...
    <div class="footer-grid">
      <div class="footer-left">
        ...
        <div class="terbilang-box">
          ...
          <div class="terbilang-value">${terbilang(quotation.totalAmount)}</div>
        </div>

        <!-- Notes -->
        <div class="notes-box">
          <div class="notes-title">Catatan / Ketentuan:</div>
      ...
      <div class="footer-right">
        ...
        <table class="totals-table">
          ...
          ${quotation.taxAmount && quotation.taxAmount > 0 ? `
            <tr class="tax-row">
              <td>Pajak (PPh 21)</td>
              <td>(${formatNum(quotation.taxAmount)})</td>
            </tr>
          ` : ''}
          <tr class="grand-total-row">
            <td>Total Penawaran</td>
            <td>Rp ${formatNum(quotation.totalAmount)}</td>
          </tr>
        </table>
...
export async function printQuotation(quotation: Quotation, publicUrl?: string) {
  const qrBase64 = await getQrCodeBase64(quotation, publicUrl);
  const html = generateQuotationHTML(quotation, qrBase64, true);
...
export async function downloadQuotationPdf(quotation: Quotation, publicUrl?: string) {
  const qrBase64 = await getQrCodeBase64(quotation, publicUrl);
  ...
  doc.setFontSize(24);
  doc.text('PENAWARAN', 195, 20, { align: 'right' });
  ...
  if (quotation.validUntil) {
    doc.setFont('helvetica', 'normal');
    doc.text('Berlaku Hingga :', 155, 35, { align: 'right' });
    doc.setFont('helvetica', 'bold');
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('PENAWARAN KEPADA', 110, partyY);
  doc.setDrawColor(15, 23, 42);
  doc.setLineWidth(0.5);
  ...
  // LEFT COLUMN: Terbilang Box & Notes Box
  // 1. Terbilang Box
  ...
  doc.setTextColor(15, 23, 42);
  doc.text('Total Penawaran', 155, rightY, { align: 'right' });
