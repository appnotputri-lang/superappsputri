...
export const generateWordDoc = async (data: CompanyData) => {
  ...
  data.shareholders.filter(sh => sh.sharesOwned > 0).forEach((sh, idx) => {
  ...
  children.push(createBodyParagraph({
    ...
    children: [
      new TextRun({ text: "Bahwa dari semua saham yang telah dikeluarkan, ditempatkan dan disetor tersebut di atas, yaitu sebanyak ", size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: totalShares.toLocaleString('id-ID'), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: ` (${numberToWords(totalShares)}) lembar saham atau senilai `, size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: formatRpDot(totalValue), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: ` (${numberToWords(totalValue)} rupiah).`, size: FONT_SIZE, font: FONT_FAMILY })
    ]
  }));
