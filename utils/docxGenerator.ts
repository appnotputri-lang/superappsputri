...
const getIdentificationStr = (sh: { nationalityType?: string, passportNumber?: string, hasKitas?: boolean, kitasNumber?: string, kitasType?: string, nik?: string }) => {
  if (sh.nationalityType === 'WNA') {
...
export const generateWordDoc = async (data: CompanyData) => {
  ...
  children.push(
    new Paragraph({
      ...
      children: [new TextRun({ 
        text: isCircular ? "KEPUTUSAN PARA PEMEGANG SAHAM SEBAGAI PENGGANTI" : "NOTULEN", 
        bold: true, size: FONT_SIZE, font: FONT_FAMILY 
      })],
    }),
    new Paragraph({
      alignment: "center" as any,
      spacing: { after: 0, line: LINE_SPACING },
      children: [new TextRun({ text: "RAPAT UMUM PEMEGANG SAHAM LUAR BIASA", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
    }),
    new Paragraph({
      alignment: "center" as any,
      children: [new TextRun({ text: `PT ${data.companyName.toUpperCase() || '................'}`, bold: true, underline: { type: 'single' }, size: FONT_SIZE, font: FONT_FAMILY })],
  ...
  if (!isCircular) {
    children.push(new Paragraph({
      spacing: { after: 240 },
      children: [new TextRun({ text: "I. RAPAT", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
    }));
    children.push(createBodyParagraph({ text: "Rapat tersebut dihadiri oleh:" }));
  } else {
  ...
  attendingShareholders.forEach((sh, idx) => {
    ...
    children.push(
      createBodyParagraph({
        ...
        children: [
          new TextRun({ text: `${sh.salutation} `, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: (sh.name || '................').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: `, lahir di ${toTitleCase(sh.birthCity || '................')}, pada tanggal ${getDayIndo(sh.birthDate) || '..'} ${getMonthIndo(sh.birthDate) || '........'} ${getYearIndo(sh.birthDate) || '....'}, ${getNationalityStr(sh)}, ${getOccupationStr(sh)}${getAddressStr(sh)}, ${getIdentificationStr(sh)};`, size: FONT_SIZE, font: FONT_FAMILY }),
        ],
      }),
      ...(isCircular 
        ? [
        ...
        : [
          createBodyParagraph({
            indent: { left: INDENT_STEP },
            children: [
              new TextRun({ text: "- dalam hal ini hadir selaku :", size: FONT_SIZE, font: FONT_FAMILY })
            ],
            spacing: { before: 60 }
          }),
          ...(sh.isManagement 
            ? [
                createBodyParagraph({
                  ...
                  children: [
                    ...
                    new TextRun({ text: `${toTitleCase(sh.managementPosition || 'Direktur')} Perseroan; dan`, size: FONT_SIZE, font: FONT_FAMILY })
                  ],
                  spacing: { after: 0 }
                })
              ]
          ...
          createBodyParagraph({
            indent: { left: INDENT_STEP + HANGING_SIZE },
            children: [
              new TextRun({ text: `${sh.isManagement ? 'b. ' : 'a. '}`, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: "Pemilik dan pemegang saham sebanyak ", size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: sh.sharesOwned.toLocaleString('id-ID'), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: ` (${numberToWords(sh.sharesOwned)}) lembar saham atau senilai `, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: formatRpDot(currentValue), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: ` (${numberToWords(currentValue)} rupiah) berhak mengeluarkan suara `, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: sh.sharesOwned.toLocaleString('id-ID'), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: ` (${numberToWords(sh.sharesOwned)}) suara dalam rapat.`, size: FONT_SIZE, font: FONT_FAMILY })
            ],
            spacing: { after: 120 }
          })
        ]
  ...
  // Listing Guests
  if (!isCircular && data.guests && data.guests.length > 0) {
    data.guests.forEach((guest) => {
      children.push(
        createBodyParagraph({
          numbering: { reference: "sh-num", level: 0 },
          children: [
            new TextRun({ text: (guest.name || '................').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: guest.position ? `, ${toTitleCase(guest.position)}` : '', size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: ';', size: FONT_SIZE, font: FONT_FAMILY }),
          ],
        })
      );
  ...
  children.push(createBodyParagraph({
    ...
    children: [
      new TextRun({ 
        ...
        size: FONT_SIZE, font: FONT_FAMILY 
      }),
      new TextRun({ text: presentShares.toLocaleString('id-ID'), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: ` (${numberToWords(presentShares)}) lembar saham atau senilai `, size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: formatRpDot(totalValue), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: ` (${numberToWords(totalValue)} rupiah)${!isCircular ? ` atau setara dengan ${attendingPercentage === 100 ? "100%" : `${attendingPercentage.toFixed(2)}%`} dari seluruh saham yang telah dikeluarkan oleh Perseroan` : ""}.`, size: FONT_SIZE, font: FONT_FAMILY })
    ]
  }));
  ...
  children.push(createBodyParagraph({
    indent: { left: INDENT_STEP },
    children: [
      new TextRun({ text: "- Untuk selanjutnya secara bersama-sama disebut sebagai ", size: FONT_SIZE, font: FONT_FAMILY }),
      new TextRun({ text: "“Para Pemegang Saham”", bold: true, size: FONT_SIZE, font: FONT_FAMILY })
    ],
    spacing: { after: 240 }
  }));
  ...
  if (isCircular) {
    ...
    children.push(new Paragraph({
      spacing: { before: 240, after: 120 },
      children: [new TextRun({ text: "DENGAN INI MENYATAKAN, bahwa Para Pemegang Saham telah mengetahui mengenai :", bold: true, size: FONT_SIZE, font: FONT_FAMILY })]
    }));
    ...
    children.push(createBodyParagraph({
      children: [
        new TextRun({ text: "OLEH KARENA ITU,", bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: " para pemegang saham secara bersama-sama setuju dan memutuskan hal-hal sebagai berikut:", size: FONT_SIZE, font: FONT_FAMILY })
      ],
      spacing: { after: 240 }
    }));
    ...
    children.push(new Paragraph({
      spacing: { before: 480, after: 240 },
      children: [new TextRun({ text: "III. AGENDA RAPAT", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
    }));
    children.push(createBodyParagraph({ text: "Rapat ini diadakan dengan agenda rapat sebagai berikut :" }));
    children.push(createBodyParagraph({
      indent: { left: INDENT_STEP },
      children: [
        new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: (data.meetingAgenda || 'Perpanjangan pengurus perseroan.'), size: FONT_SIZE, font: FONT_FAMILY })
      ]
    }));
    ...
    children.push(new Paragraph({
      spacing: { before: 480, after: 240 },
      children: [new TextRun({ text: "IV. DETAIL RAPAT DAN AKTA PENDIRIAN/PERUBAHAN", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
    }));

    const invitationDateText = formatDateIndo(data.invitationDate) || '................';
    children.push(createBodyParagraph({
      children: [
        new TextRun({ text: `Rapat ini diselenggarakan berdasarkan Surat Undangan Direksi PT. ${data.companyName.toUpperCase()} Nomor : `, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: data.invitationNumber || '................', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: " tanggal ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: invitationDateText, bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: ", dan diadakan pada:", size: FONT_SIZE, font: FONT_FAMILY })
      ]
    }));
    ...
    children.push(
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: `Hari/Tanggal\t: ${dayName}, ${dateText}`, tabStops: [{ type: 'left' as any, position: 2000 }] }),
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: `Tempat\t: ${data.signingPlace || '................'}`, tabStops: [{ type: 'left' as any, position: 2000 }] }),
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: `Waktu\t: Pukul ${data.meetingStartTime || '..:..'} WIB`, tabStops: [{ type: 'left' as any, position: 2000 }], spacing: { after: 240 } })
    );

    children.push(createBodyParagraph({ text: "Adapun Anggaran Dasar Perseroan, adalah sebagai berikut :" }));
    children.push(createBodyParagraph({
      children: [
        new TextRun({ text: "Bahwa Perseroan didirikan dengan Akta Pendirian tertanggal ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: formatDateIndo(data.establishmentDeedDate) || '..........', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: ", No. ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: data.establishmentDeedNumber || '..........', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: ", yang dibuat dihadapan ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: `${data.establishmentNotary || '..........'}${data.establishmentNotaryTitle ? `, ${data.establishmentNotaryTitle}` : ''}`, bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: `, Notaris di ${data.oldAddress?.city || '..........'} dan telah mendapat pengesahan dari Menteri Hukum dan Hak Asasi Manusia Republik Indonesia tertanggal `, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: formatDateIndo(data.establishmentSkDate) || '..........', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: ", Nomor ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: data.establishmentSkNumber || '..........', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: data.amendmentDeeds && data.amendmentDeeds.length > 0 ? " beberapa kali telah mengalami perubahan, berdasarkan :" : " .", size: FONT_SIZE, font: FONT_FAMILY })
      ]
    }));
    ...
    if (data.amendmentDeeds && data.amendmentDeeds.length > 0) {
      data.amendmentDeeds.forEach(deed => {
        children.push(createBodyParagraph({
          ...
          children: [
            new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: "Akta Pernyataan Keputusan Rapat Umum Para Pemegang Saham Luar Biasa tertanggal ", size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: formatDateIndo(deed.date) || '..........', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: " Nomor ", size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: deed.number || '..........', bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: ", yang dibuat di hadapan ", size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: `${deed.notary || '..........'}${deed.notaryTitle ? `, ${deed.notaryTitle}` : ''}`, bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
            new TextRun({ text: `, Notaris di ${data.oldAddress?.city || '..........'} `, size: FONT_SIZE, font: FONT_FAMILY }),
            ...(deed.skSpDocuments || []).map((doc, dIdx) => (
              new TextRun({ 
    ...
    children.push(new Paragraph({
      spacing: { before: 480, after: 240 },
      children: [new TextRun({ text: "V. JALANNYA RAPAT", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
    }));
    children.push(createBodyParagraph({
  ...
  // --- RESOLUTIONS TITLE ---
  const resolutionsLabel = isCircular ? "KEPUTUSAN-KEPUTUSAN" : "VI. KEPUTUSAN-KEPUTUSAN";
  children.push(new Paragraph({
    spacing: { before: 480, after: 240 },
    children: [new TextRun({ text: resolutionsLabel, bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
  }));
  ...
  const addResolution = (resTitle: string, resBody: any[]) => {
    children.push(createBodyParagraph({
      ...
      children: [new TextRun({ text: resTitle, bold: true, size: FONT_SIZE, font: FONT_FAMILY })]
    }));
  ...
  if (data.resolutions.domicile || data.resolutions.address) {
    ...
    addResolution("Persetujuan Perubahan Kedudukan/Alamat Perseroan", [
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: domicileText })
    ]);
  }

  if (data.resolutions.kbli) {
    addResolution("Persetujuan Perubahan Maksud dan Tujuan Perseroan", [
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: "Menyetujui dan memutuskan untuk mengubah ketentuan Pasal 3 ayat (1) dan ayat (2) Anggaran Dasar Perseroan mengenai Maksud dan Tujuan serta Kegiatan Usaha, sehingga selanjutnya menjadi berbunyi sebagai berikut :" })
    ]);
  }
  ...
  if (data.resolutions.capitalBase || data.resolutions.capitalBaseDecrease) {
    ...
    addResolution(data.resolutions.capitalBaseDecrease ? "Persetujuan Penurunan Modal Dasar Perseroan" : "Persetujuan Peningkatan Modal Dasar Perseroan", [
      createBodyParagraph({ 
        indent: { left: INDENT_STEP }, 
  ...
  if (data.resolutions.capitalPaid || data.resolutions.capitalPaidDecrease) {
    ...
    if (data.resolutions.capitalPaid) {
      elements.push(
        createBodyParagraph({ indent: { left: INDENT_STEP }, spacing: { before: 120 }, text: "Bahwa pengeluaran saham-saham baru tersebut di atas, telah diambil bagian dan disetor penuh secara tunai melalui kas Perseroan oleh masing - masing pemegang saham dengan rincian sebagai berikut :" })
      );
      ...
      newDeposits.forEach((dep, idx) => {
        elements.push(
          createBodyParagraph({
            ...
            children: [
              new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: `${dep.name.toUpperCase()} `, bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
              new TextRun({ text: `: ${formatInputNumber(dep.addedShares)} (${numberToWords(dep.addedShares)}) lembar saham atau senilai Rp. ${formatInputNumber(dep.addedValue)},- (${numberToWords(dep.addedValue)} rupiah);`, size: FONT_SIZE, font: FONT_FAMILY })
            ]
          })
        );
      });
    ...
    addResolution(data.resolutions.capitalPaidDecrease ? "Persetujuan Penurunan Modal Ditempatkan dan Disetor" : "Persetujuan Peningkatan Modal Ditempatkan dan Disetor", elements);
  }
  ...
  if (data.resolutions.shareholders && data.shareTransfers && data.shareTransfers.length > 0) {
    const transferBody: any[] = [
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: "Menyetujui pengalihan seluruh saham secara hibah/jual beli dengan rincian sebagai berikut :" })
    ];
    ...
    data.shareTransfers.forEach((t, i) => {
       ...
       if (fromSh && toSh) {
         transferBody.push(createBodyParagraph({
           indent: { left: INDENT_STEP * 2, hanging: HANGING_SIZE },
           children: [
             new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
             new TextRun({ text: (fromSh.name || '.....').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
             new TextRun({ text: ` mengalihkan sejumlah ${t.sharesTransferred.toLocaleString('id-ID')} (${numberToWords(t.sharesTransferred)}) saham perseroan atau senilai Rp. ${formatInputNumber(t.sharesTransferred * data.originalSharePrice)},- (${numberToWords(t.sharesTransferred * data.originalSharePrice)} rupiah) kepada `, size: FONT_SIZE, font: FONT_FAMILY }),
             new TextRun({ text: (toSh.name || '.....').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
             new TextRun({ text: `;`, size: FONT_SIZE, font: FONT_FAMILY })
           ]
         }));
       }
  ...
  if (data.resolutions.capitalBase || data.resolutions.capitalPaid || data.resolutions.capitalBaseDecrease || data.resolutions.capitalPaidDecrease || data.resolutions.shareholders) {
    ...
    data.finalShareholders.filter(s => s.sharesOwned > 0).forEach(s => {
      ...
      finalCompBody.push(createBodyParagraph({
        indent: { left: INDENT_STEP * 2, hanging: HANGING_SIZE },
        children: [
          new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: (s.name || '.....').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: ` : `, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: s.sharesOwned.toLocaleString('id-ID'), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: ` (${numberToWords(s.sharesOwned)}) lembar saham atau senilai Rp. `, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: formatRpDot(currentValue), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
          new TextRun({ text: `,- (${numberToWords(currentValue)} rupiah);`, size: FONT_SIZE, font: FONT_FAMILY })
        ]
      }));
  ...
  if (data.resolutions.management || data.resolutions.reappointment) {
    ...
    const managers = data.finalShareholders.filter(s => s.isManagement);
    const body: any[] = [
      createBodyParagraph({ indent: { left: INDENT_STEP }, text: "Menyetujui untuk memberhentikan dengan hormat seluruh anggota Direksi dan Dewan Komisaris Perseroan yang menjabat saat ini, yaitu :" })
    ];

    (data.oldManagementItems || []).forEach(m => {
       body.push(createBodyParagraph({
         indent: { left: INDENT_STEP * 2, hanging: HANGING_SIZE },
         children: [
           new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
           new TextRun({ text: (m.name || '..........').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
           new TextRun({ text: `, selaku ${toTitleCase(m.position)} perseroan;`, size: FONT_SIZE, font: FONT_FAMILY })
         ]
       }));
    });
    ...
    managers.forEach(m => {
       body.push(createBodyParagraph({
         ...
         children: [
           new TextRun({ text: "- ", size: FONT_SIZE, font: FONT_FAMILY }),
           new TextRun({ text: (m.name || '..........').toUpperCase(), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
           new TextRun({ text: `, sebagai ${toTitleCase(m.managementPosition || 'Direktur')} perseroan;`, size: FONT_SIZE, font: FONT_FAMILY })
         ]
       }));
    });
  ...
  addResolution("Pemberian Kuasa", [
    createBodyParagraph({
      indent: { left: INDENT_STEP },
      children: [
        new TextRun({ text: "Menyetujui dan memutuskan untuk memberikan kuasa dengan hak substitusi kepada ", size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: repText, bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: ", untuk melakukan tindakan-tindakan yang diperlukan sehubungan dengan keputusan Rapat di atas, termasuk memberi keterangan-keterangan, membuat, minta dibuatkan dan menandatangani segala surat dan akta dihadapan Notaris dan umumnya menjalankan segala tindakan yang dianggap perlu dan berguna, tidak ada tindakan yang dikecualikan.", size: FONT_SIZE, font: FONT_FAMILY })
      ]
    })
  ]);
  ...
  if (isCircular) {
    ...
    children.push(new Paragraph({
      ...
      children: [new TextRun({ text: "VII. PENUTUP", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
    }));

    children.push(createBodyParagraph({
      children: [
        new TextRun({ text: `Akhirnya, oleh karena sudah tidak ada hal-hal lain yang perlu dibicarakan lagi, maka Ketua Rapat menutup Rapat ini pada jam `, size: FONT_SIZE, font: FONT_FAMILY }),
        new TextRun({ text: (data.meetingEndTime || '..:..'), bold: true, size: FONT_SIZE, font: FONT_FAMILY }),
  ...
  children.push(new Paragraph({
    ...
    spacing: { after: 480, line: LINE_SPACING },
    children: [new TextRun({ text: "TANDA TANGAN PARA PEMEGANG SAHAM,", bold: true, size: FONT_SIZE, font: FONT_FAMILY })],
  }));
  ...
  attendingShareholders.forEach((sh, idx) => {
    if (idx === 0) {
      children.push(new Paragraph({
        alignment: "left" as any,
        spacing: { after: 480, line: LINE_SPACING },
        children: [new TextRun({ text: "Meterai 10.000 + Cap", color: "bfbfbf", size: 16, font: FONT_FAMILY })],
      }));
    } else {
      children.push(new Paragraph({ alignment: "left" as any, spacing: { before: 480 }, children: [] }));
    }
    ...
    children.push(
      new Paragraph({
        alignment: "left" as any,
        spacing: { after: 0, line: LINE_SPACING },
        children: [new TextRun({ text: (sh.name || '................').toUpperCase(), bold: true, underline: { type: 'single' }, size: FONT_SIZE, font: FONT_FAMILY })],
      }),
      new Paragraph({
        alignment: "left" as any,
  ...
  const doc = new Document({ 
    numbering: {
      config: [
        { reference: "sh-num", levels: [ { level: 0, format: "decimal", text: "%1.", alignment: "left" as any, style: { paragraph: { indent: { left: INDENT_STEP, hanging: HANGING_SIZE } } } } ] },
        { reference: "decl-num", levels: [ { level: 0, format: "decimal", text: "%1.", alignment: "left" as any, style: { paragraph: { indent: { left: INDENT_STEP, hanging: HANGING_SIZE } } } } ] },
        { reference: "res-num", levels: [ { level: 0, format: "decimal", text: "%1.", alignment: "left" as any, style: { paragraph: { indent: { left: INDENT_STEP, hanging: HANGING_SIZE } } } } ] },
      ],
    },
    sections: [{ 
      properties: { page: { margin: { top: MARGIN_NORMAL, bottom: MARGIN_NORMAL, left: MARGIN_NORMAL, right: MARGIN_NORMAL } } },
      children 
    }] 
  });
